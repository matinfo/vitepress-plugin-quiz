...
`@matinfo/vitepress-plugin-quiz` ships with five built-in locales and a simple API to use or override them.
...
```ts
import { getLocale, locales } from "@matinfo/vitepress-plugin-quiz"
...
```ts
import { enhanceAppWithQuiz } from "@matinfo/vitepress-plugin-quiz"
import type { QuizLocale } from "@matinfo/vitepress-plugin-quiz"
